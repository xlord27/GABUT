...
VALIDITY () {
    ...
    Exp1=$(curl https://raw.githubusercontent.com/xlord27/permission/main/ipmini | grep $MYIP | awk '{print $4}')
    if [[ $today < $Exp1 ]]; then
...
IZIN=$(curl https://raw.githubusercontent.com/xlord27/permission/main/ipmini | awk '{print $5}' | grep $MYIP)
if [ $MYIP = $IZIN ]; then
