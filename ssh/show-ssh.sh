...
user=$(grep -E "^#sh# " "/etc/ssh/.ssh.db") | cut -d ' ' -f 2 | sed -n "${CLIENT_NUMBER}"p)
harini=$(grep -E "^#sh# " "/etc/ssh/.ssh.db") | cut -d ' ' -f 4 | sed -n "${CLIENT_NUMBER}"p)
exp=$(grep -E "^#sh# " "/etc/ssh/.ssh.db") | cut -d ' ' -f 3 | sed -n "${CLIENT_NUMBER}"p)
uuid=$(grep -E "^#sh# " "/etc/ssh/.ssh.db") | cut -d ' ' -f 5 | sed -n "${CLIENT_NUMBER}"p)
if [[ ${c} != "0" ]]; then
  echo "${d}" >/etc/ssh/${LOGIN}
fi
DATADB=$(cat /etc/ssh/.ssh.db | grep "^#sh#" | grep -w "${LOGIN}" | awk '{print $2}')
if [[ "${DATADB}" != '' ]]; then
  sed -i "/\b${LOGIN}\b/d" /etc/ssh/.ssh.db
fi
echo "#sh# ${LOGIN} " >>/etc/ssh/.ssh.db
